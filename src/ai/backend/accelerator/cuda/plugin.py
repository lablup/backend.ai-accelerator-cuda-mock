...
class CUDAPlugin(AbstractComputePlugin):
    ...
    async def available_slots(self) -> Mapping[SlotName, Decimal]:
        ...
        if self._mode == AllocationModes.DISCRETE:
            slots[SlotName('cuda.device')] = Decimal(len([
                dev for dev in devices
                if not dev.is_mig_device
            ]))
        elif self._mode == AllocationModes.FRACTIONAL:
            slots[SlotName('cuda.shares')] = Decimal(sum(
                self._get_share(dev) for dev in devices
                if not dev.is_mig_device
            ))
        for dev in devices:
            if not dev.is_mig_device:
    ...
    async def gather_node_measures(self, ctx: StatContext) -> Sequence[NodeMeasurement]:
        ...
        if self.enabled:
            for device_info in self._all_devices:
                ...
                mem_stats[device_id] = Measurement(Decimal(dev_stat.mem_used),
                ...
                power_usage[device_id] = Measurement(Decimal(dev_stat.power_usage // 1000),
                                                     Decimal(dev_stat.power_max // 1000))
                temperature[device_id] = Measurement(Decimal(dev_stat.core_temperature),
                                                     Decimal(120))
    ...
    async def create_alloc_map(self) -> AbstractAllocMap:
        ...
        if self._mode == AllocationModes.DISCRETE:
            return DiscretePropertyAllocMap(
                device_slots={
                    dev.device_id: (
                        ...
                        if not dev.is_mig_device else
                        DeviceSlotInfo(SlotTypes.UNIQUE, self._get_mig_slot_name(dev), Decimal(1))
                    ) for dev in devices
                },
            ...
            return FractionAllocMap(
                device_slots={
                    dev.device_id: (
                        DeviceSlotInfo(SlotTypes.COUNT, SlotName('cuda.shares'), self._get_share(dev))
                        if not dev.is_mig_device else
                        DeviceSlotInfo(SlotTypes.UNIQUE, self._get_mig_slot_name(dev), Decimal(1))
                    ) for dev in devices
                },
    ...
    async def generate_resource_data(
        ...
        data['CUDA_RESOURCE_VIRTUALIZED'] = '0' \
            if is_unique or self._mode == AllocationModes.DISCRETE \
            else '1'
        if self._mode == AllocationModes.FRACTIONAL:
            mem_limits = []
            proc_limits = []
            ...
            mlim_str = ','.join(
                f'{device_id}:{mem: }' for device_id, mem in
                mem_limits
            )
            plim_str = ','.join(
                f'{device_id}:{proc}' for device_id, proc in
                proc_limits
            )
